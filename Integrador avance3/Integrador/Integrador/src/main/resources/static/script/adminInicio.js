...
const paginasBonitas = {
    "/": "PAGINA PRINCIPAL (Donde el usuario ve la información)",
    "/miCuenta": "CUENTA DE USUARIO",
    "/iniciar-sesion": "PAGINA DE INICIO DE SESIÓN",
    "/tienda": " TIENDA (donde se realizan las compras / solicita visitas técnicas) ",
};
...
async function cargarAnalytics() {
    ...
    try {
        ...
        document.getElementById("usuariosActivos").innerText = activos;

        
        const tbody = document.getElementById("tablaPaginas");
        tbody.innerHTML = "";

   paginas.forEach(p => {

    const nombreBonito = paginasBonitas[p.pagina] || p.pagina;
    ...
    tr.innerHTML = `
        <td class="fw-bold text-dark">
            ${nombreBonito}
            <br>
            <small>${p.pagina}</small>
        </td>
        <td class="fw-bold text-dark">${p.visitas}</td>
    `;
